...
class Settings:
    ...
    def start_menu(self, window):
        while True:
            ...
            pg.display.set_caption(f'Snake {self.ver} Level: {settings.level}. Walls: {self.walls}, RGBSnake: {self.rainbow}.')
        ...
        if self.next == True:
            ...
            for s_xy in self.tails_pos:
                ...
                if len(self.apple_pos) > 0:
                    for i, apple in enumerate(self.apple_pos):
                        if s_xy == self.apple_pos[i]:
                            self.tail_snake = pg.draw.rect(self.window, (255, 100, 0),
                    ...
                    self.tail_snake = pg.draw.rect(self.window, settings.snake_color,
                                                [s_xy[0], s_xy[1], self.square_size-1, self.square_size-1],
                                                 border_radius = 2)
            if len(self.apple_pos) > 0:
                    if self.tails_pos[-1] == self.apple_pos[0]:
...
if __name__ == '__main__':
    ...
    while settings.start:
        if snake.walls == True:
            pg.display.set_caption(f'Score: {settings.score} Level: {settings.level}. Snake speed:{round(settings.fps, 1)}. Walls.')
        elif snake.walls == False:
            pg.display.set_caption(f'Score: {settings.score} Level: {settings.level}. Snake speed:{round(settings.fps, 1)}.')
        snake.snake_draw()
